...
//   - checkout.session.expired    (DISMESSO il 2026-09-23: nessuna email di recupero, risponde solo 200;
//                                   l'evento può essere disiscritto dal dashboard Stripe)
...
function prearrivalHtml(m) {
  ...
  <ul style="padding-left:1.1rem">
    ...
    <li><b>⚠️ Importante:</b> l'appartamento è al <b>3° piano e mezzo, senza ascensore</b>.</li>
  ...
  <ul style="padding-left:1.1rem">
    ...
    <li><b>⚠️ Please note:</b> the flat is <b>three and a half floors up, with no lift</b>.</li>
...
export default async function handler(req, res) {
  ...
  // --- Recupero checkout abbandonato: DISMESSO il 2026-09-23 ---
  // Motivo: il pagamento online con carta è stato tolto dal sito (api/checkout.js risponde 410),
  // quindi non ha più senso invitare l'ospite a "riprendere" un checkout Stripe. Rispondiamo 200
  // senza inviare nulla (evita retry da Stripe finché l'evento resta iscritto sul dashboard).
  if (event.type === 'checkout.session.expired') {
    return res.status(200).json({ received: true, recovery: 'disabled' });
  }
